exports.seed = async function (knex, promise) {
  ...
  if (eventTypeCountInt != expectedEventTypeCount) {
    errorMessage += `event_type table has ${expectedEventTypeCount} records, but should have ${eventTypeCountInt} records. `;
  }
  if (expectedPersonCount != personCountInt) {
    errorMessage += `person table has ${personCountInt} records, but should have ${expectedPersonCount} records. `;
  }
  if (expectedAssignmentCount > 450 || expectedAssignmentCount < 350) {
    errorMessage += `assignment table has ${assignmentCountInt} records, but should have around 400 records.`;
  }
  if (expectedEventCount > 375 || eventCountInt < 325) {
    errorMessage += `event table has ${eventCountInt} records, but should have around 340 records. `;
  }
